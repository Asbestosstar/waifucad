...
extern(C) int main()
{
    ...
    /* Model is several megabytes; heap allocation keeps the test
       frame small and avoids a huge default-initialiser constant. */
    auto redundant=cast(Model*)malloc(Model.sizeof); if(redundant is null)return 90;
    redundant.initialise("redundant".ptr);
    ...
    /* Model is several megabytes; heap allocation keeps the test
       frame small and avoids a huge default-initialiser constant. */
    auto tangent=cast(Model*)malloc(Model.sizeof); if(tangent is null)return 90;
    tangent.initialise("tangent".ptr);
    ...
    /* Model is several megabytes; heap allocation keeps the test
       frame small and avoids a huge default-initialiser constant. */
    auto curveTangent=cast(Model*)malloc(Model.sizeof); if(curveTangent is null)return 90;
    curveTangent.initialise("curve-tangent".ptr);
    ...
    /* Model is several megabytes; heap allocation keeps the test
       frame small and avoids a huge default-initialiser constant. */
    auto symmetric=cast(Model*)malloc(Model.sizeof); if(symmetric is null)return 90;
    symmetric.initialise("symmetry".ptr);
    ...
    /* Model is several megabytes; heap allocation keeps the test
       frame small and avoids a huge default-initialiser constant. */
    auto conflicting=cast(Model*)malloc(Model.sizeof); if(conflicting is null)return 90;
    conflicting.initialise("conflict".ptr);
    ...
    /* Model is several megabytes; heap allocation keeps the test
       frame small and avoids a huge default-initialiser constant. */
    auto cancelled=cast(Model*)malloc(Model.sizeof); if(cancelled is null)return 90;
    cancelled.initialise("cancelled".ptr);
